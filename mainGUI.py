...
class AppCamera:
    ...
    def __init__(self, window, window_title):
        ...
        self.ImBg = cv2.imread('img/Fondo (2).png')
        ...
        self.random_videos_button = customtkinter.CTkButton(window, text="Start Random Videos", command=self.toggle_random_videos)
        self.random_videos_button.pack(pady=10)
    ...
    def toggle_random_videos(self):
        if self.running.is_set():
            self.running.clear()
            self.random_videos_button.configure(text="Start Random Videos")
        else:
            self.running.set()
            self.random_videos_button.configure(text="Stop Random Videos")
            Thread(target=self.process_random_videos, daemon=True).start()

    def process_random_videos(self):
        dataset_path = "DATASET-T"  # Path to your dataset folder
        count = 0
    ...
    def process_video(self, video_path):
        ...
        pTime = 0
        cTime = 0
        while cap.isOpened() and self.running.is_set():
            ret, frameVisual = cap.read()
            if not ret:
                break
            #time.sleep(0.001)
    ...
    def createGIF(self):
        ...
        imageio.mimwrite('img/Caida2.gif', self.GIF, 'GIF', duration=1, fps=10) 
        print('GIF guardado')
        Thread(target=self.sendAlert, daemon=True).start()
        
    def sendAlert(self):
        try:
            response = requests.post(self.RECEIVE_GIF_URL, files={'file': open('img/Caida2.gif', 'rb')})
